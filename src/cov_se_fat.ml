...
module Params = struct
  ...
  let create (params : params) =
    let check v_dim name v =
      ...
      if n <> params.d then
        failwithf
          "Cov_se_fat.Params.create: %s projection (%d) disagrees with target \
           dimension d (%d)"
          name n params.d ()
    in
...
module Eval = struct
  ...
  module Input = struct
    ...
    let eval k input inducing =
      let { Kernel.multiscales; params = { Params.d; log_sf2; tproj } } = k in
      let projection =
        match tproj with
        | None -> input
  ...
  module Inputs = struct
    ...
    let create_default_kernel_params inputs ~n_inducing =
      ...
      for r = 1 to big_dim do
        ...
        for c = 1 to n_inputs do
          sum_ref := !sum_ref +. inputs.{r, c}
        done;
...
module Proj_hyper = struct
  type t = { big_dim : int; small_dim : int }
end

module Dim_hyper = struct
  type t = int
end

module Inducing_hyper = struct
...
module Deriv = struct
  ...
  module Hyper = struct
    ...
    let get_all { Eval.Kernel.params } inducing _inputs =
      let { Params.d; tproj; log_hetero_skedasticity; log_multiscales_m05 } =
        params
      in
      let m = Mat.dim2 inducing in
    ...
    let get_value { Eval.Kernel.params } inducing _inputs = function
      ...
      | `Log_multiscale_m05 { Inducing_hyper.ind; dim } ->
          (option_get_value "log_multiscales_m05"
             params.Params.log_multiscales_m05).{dim, ind}
    ...
    let set_values { Eval.Kernel.params } inducing inputs hypers values =
      ...
      let log_hetero_skedasticity_lazy =
        lazy_opt "log_hetero_skedasticity" copy
      ...
      let lift_opt lazy_value value =
        if Lazy.is_val lazy_value then Some (Lazy.force lazy_value) else value
      in
        ...
        Eval.Kernel.create
          {
            ...
            log_hetero_skedasticity =
              lift_opt log_hetero_skedasticity_lazy
                params.Params.log_hetero_skedasticity;
            log_multiscales_m05 =
              lift_opt log_multiscales_m05_lazy
                params.Params.log_multiscales_m05;
          }
      in
  ...
  module Inducing = struct
    ...
    let calc_deriv_upper (inducing, { kernel; eval_mat }) hyper =
      ...
      | `Log_hetero_skedasticity dim -> (
          ...
          | None ->
              failwith
                "Cov_se_fat.Deriv.Inducing.calc_deriv_upper: heteroskedastic \
                 modeling disabled, cannot calculate derivative"
          | Some hetero_skedasticity ->
              ...
              `Diag_vec deriv)
      | `Log_multiscale_m05 { Inducing_hyper.ind; dim } -> (
          match kernel.Eval.Kernel.multiscales with
          | None ->
              failwith
                "Cov_se_fat.Deriv.Inducing.calc_deriv_upper: multiscale \
                 modeling disabled, cannot calculate derivative"
          | Some multiscales ->
  ...
  module Inputs = struct
    ...
    let check_tproj_available = function
      | None ->
          failwith
            "Cov_se_fat.Deriv.Inputs.calc_deriv_cross: tproj disabled, cannot \
             calculate derivative"
      | Some _ -> ()

    let calc_deriv_cross cross hyper =
      let { Cross.common = { kernel; eval_mat }; inputs; inducing; projections }
          =
        cross
      in
      match hyper with
      ...
      | `Log_multiscale_m05 { Inducing_hyper.ind; dim } -> (
          ...
          | None ->
              failwith
                "Cov_se_fat.Deriv.Inputs.calc_deriv_cross: multiscale modeling \
                 disabled, cannot calculate derivative"
          | Some multiscales ->
