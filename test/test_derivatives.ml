...
let main () =
  ...
  let params =
    Cov_se_fat.Eval.Inputs.create_default_kernel_params ~n_inducing
      training_inputs
  in
  ...
  in
  let sigma2 = 1. in
  FITC.Deriv.Test.self_test kernel inducing_points training_inputs ~sigma2
    ~targets:training_targets `Sigma2;
  Array.iter all_hypers ~f:(fun hyper ->
      ...
      FITC.Deriv.Test.check_deriv_hyper kernel inducing_points training_inputs
        hyper;
      FITC.Deriv.Test.self_test kernel inducing_points training_inputs ~sigma2
        ~targets:training_targets (`Hyper hyper))
